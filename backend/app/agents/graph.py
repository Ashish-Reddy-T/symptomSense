...
def create_agent_graph(*, settings: Settings, vit: ViTImageClassifier):
    """Create and configure the LangGraph for agent orchestration."""

    # LLM
    decision_model = settings.GEMINI_MODEL
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        if isinstance(current_input, dict) and "image" in current_input:
            ...
            image_type_response = vit.classify_image(image_path)
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        if decision["confidence"] < settings.MIN_IMAGE_CONFIDENCE:
    ...
    def run_conversation_agent(state: AgentState) -> AgentState:
        ...
        response = settings.GEMINI_MODEL.invoke(conversation_prompt)
    ...
    def run_rag_agent(state: AgentState) -> AgentState:
        ...
        rag_context_limit = settings.max_conversation_history
        ...
        if retrieval_confidence >= settings.MIN_IMAGE_CONFIDENCE:
    ...
    def run_web_search_processor_agent(state: AgentState) -> AgentState:
        ...
        messages = state["messages"]
        web_search_context_limit = settings.max_conversation_history
        ...
        web_search_processor = WebSearchProcessorAgent(settings)
    ...
    def confidence_based_routing(state: AgentState) -> str:
        ...
        if (state.get("retrieval_confidence", 0.0) < settings.MIN_IMAGE_CONFIDENCE or 
    ...
    def run_chest_xray_agent(state: AgentState) -> AgentState:
        ...
        predicted_class = vit.classify_chest_xray(image_path)
    ...
    def run_skin_lesion_agent(state: AgentState) -> AgentState:
        ...
        # classify chest x-ray into covid or normal
        predicted_mask = vit.segment_skin_lesion(image_path)
...
def build_graph(
    *,
    ...
    # Instantiate the agent decision subgraph
    agent_decision_subgraph = create_agent_graph(settings=settings, vit=vit)

    # Add the subgraph as a node
    graph.add_node("agent_decision_subgraph", agent_decision_subgraph)

    # Set the entry point to the agent decision subgraph
    graph.set_entry_point("agent_decision_subgraph")

    # The agent_decision_subgraph will handle all the routing and final output generation
    # So, the main graph will just end after the subgraph completes.
    graph.add_edge("agent_decision_subgraph", END)
